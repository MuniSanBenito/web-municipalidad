...
export function ContinueButton({ onClick, label = 'Continuar', children }: PropsWithChildren<ContinueButtonProps>) {
  return (
    <motion.div initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ type: 'spring', stiffness: 400, damping: 18 }} className="mt-4">
      <motion.button
        {...buttonTap}
        onClick={onClick}
        className="btn btn-lg w-full max-w-xs rounded-2xl border-none bg-primary text-primary-content shadow-md hover:shadow-lg sm:w-auto sm:min-w-[200px]"
      >
        {children ?? label}
