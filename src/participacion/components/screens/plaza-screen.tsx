...
export function PlazaScreen({ elements, onComplete }: PlazaScreenProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={staggerItem}
        className="mb-4 flex min-h-[80px] w-full max-w-md flex-wrap items-center justify-center gap-2 rounded-2xl border-2 border-primary/20 bg-primary/10 p-4"
      >
        {selected.length === 0 ? (
          ...
          elements
            ...
            .map((el) => (
              <motion.div
                ...
                className="flex items-center gap-1.5 rounded-2xl bg-base-100 px-3 py-2 shadow-sm"
              >
      ...
      {selected.length > 0 && (
        <motion.button
          ...
          className="btn btn-md mt-4 min-w-[160px] rounded-2xl border-none bg-primary text-primary-content shadow-md hover:shadow-lg"
        >
